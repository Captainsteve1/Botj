...
DEFAULTS = {
    'esn': MSL_UTILS.generate_esn('NFCDCH-02-'),
    'drm_system': 'widevine',
    'profiles': [
...
class MslClient:
        ...
        try:
            ...
            if manifest.get("error"):
                if manifest["error"].get("display"):
                    raise ManifestError(manifest["error"]["display"])
                else:
                    raise ManifestError(manifest["error"])

            if manifest.get("errormsg"):
                raise ManifestError(manifest["errormsg"])
        ...
        try:
            ...
            if msl_license_data.get("error"):
                if msl_license_data["error"].get("display"):
                    raise LicenseError(msl_license_data["error"]["display"])
                else:
                    raise LicenseError(msl_license_data["error"])
            if msl_license_data.get("errormsg"):
                raise LicenseError(msl_license_data["errormsg"])
    ...
    def perform_key_handshake(self):
        ...
        header['headerdata'] = base64.b64encode(
            MSL_UTILS.dumps(self.header).encode('utf8')
        ).decode('utf8')
        ...
        payload['payload'] = base64.b64encode(MSL_UTILS.dumps({
            'sequencenumber': 1,
            'messageid': self.msl_session['message_id'],
            'endofmsg': True,
            'data': ''
        ...
        request = MSL_UTILS.dumps(header) + MSL_UTILS.dumps(payload)
        resp = requests.post(
            url=ENDPOINTS['manifest'],
            data=request,
            proxies=self.msl_session['proxies']
    ...
    def parse_handshake(self, response):
        ...
        encryption_key = MSL_UTILS.webcrypto_b64decode(
        ...
        sign_key = MSL_UTILS.webcrypto_b64decode(sign_key_data['k'])
    ...
    def generate_msl_request_data(self, data):
        ...
        header_envelope = MSL_UTILS.msl_encrypt(
            self.msl_session, MSL_UTILS.dumps(header)
        )
        ...
        payload = {
            ...
            'data': base64.b64encode(
                MSL_UTILS.dumps(data).encode('utf8')
            ).decode('utf8'),
            'sequencenumber': 1,
            'endofmsg': True
        }

        payload_envelope = MSL_UTILS.msl_encrypt(
            self.msl_session,
            MSL_UTILS.dumps(payload)
        )
        ...
        return MSL_UTILS.dumps(enc_header) + MSL_UTILS.dumps(payload_chunk)
