...
import os
import time
from helpers.Parsers.netflix.remote_device import RemoteDevice
...
API_HOST = 'http://34.127.114.170:7010/'
API_KEY = '90beb74575b52ed3fb2dfb0dfea112a9'
remote_device = RemoteDevice(API_HOST, API_KEY)
...
class License:
    ...
    def get_keys(self,):
        ...
        LICENSE_PROFILES += (
            self.Profiles.get(profile=self.profile, resolution="HD")
            if self.profile == "MAIN"
        ...
        (session_id, challenge) = remote_device.get_license_challenge(init_data_b64)
        current_sessionId = str(time.time()).replace(".", "")[0:-2]
        data = client.get_license(
            challenge, current_sessionId
        )
        try:
            license_b64 = data["result"][0]["licenseResponseBase64"]
        except Exception:
            exit("MSL LICENSE Error Message: {}".format(data))
        keys =remote_device.get_keys(license_b64, session_id)
