...
def get_metadata_seasons(url: str, data: str) -> List[Dict[str, Any]]:
    try:
        return [{"title_id": get_title_id(item.get("link")), "season_number": correct_season_n(item.get("sequenceNumber")
...
def metadata_parser(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _type == "season":
        ...
        for _data in data:
            ...
            } for episode in [episodes for episodes in _data["widgets"]["titleContent"] if episodes["collectionType"] == "Episodes"][0]["cards"]]
...
def get_metadata(url: str, cookie: str):
    ...
    for season in seasons:
        ...
        if os.path.isfile((cached := f"{title_id}_s_{season_number}.json")):
            data.append(json.load(open(cached)))
            continue

        print("Getting Season metadata ID = %s | NUMBER = %d" % (title_id, season_number))
