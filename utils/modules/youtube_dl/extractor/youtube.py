    ...
    def _real_extract(self, url):
        ...
        if 'conn' in video_info and video_info['conn'][0].startswith('rtmp'):
            ...
            for fmt in streaming_formats:
                ...
                language = fmt.get('audioTrack').get('id') if fmt.get('audioTrack') else None
                if language:
                    if "." in language:
        ...
        try:
            licenseInfos = player_response["streamingData"]["licenseInfos"]
            wv_license = [x["url"] for x in licenseInfos if x["drmFamily"] == "WIDEVINE"][0]
        except Exception:
            licenseInfos = None
            wv_license = None
