...
def rename(file, source, group):
    ...
    for track in media_info.tracks:        
        if track.track_type == 'Video':            
            ...
            if track.format == "AVC": codec = "H.264"
            elif track.format == "HEVC": codec = "H.265"
        ...
        if track.track_type == 'Audio':
            if track.format == "E-AC-3": audioCodec = "DD+"
            elif track.format == "AC-3": audioCodec = "DD"
            elif track.format == "AAC": audioCodec = "AAC"
            else:
                print("No Audio Root Found: {}".format(track.format))
                audioCodec = None                
            
            if track.channel_s == 6: channels = "5.1"
